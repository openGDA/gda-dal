 ...
 *
 */
public abstract class AbstractPropertyFactory extends AbstractFactorySupport implements PropertyFactory{
	...
	private DynamicValueProperty<?> createProperty (String uniqueName, Class<? extends DynamicValueProperty<?>> type, LinkListener<?> l) throws RemoteException{
		...
		try {
			...
			Class<?extends SimpleProperty<?>> impClass = getPlugInstance().getPropertyImplementationClass(type, uniqueName);
			DynamicValuePropertyImpl<?> property = (DynamicValuePropertyImpl<?>)impClass.getConstructor(String.class, PropertyContext.class).newInstance(uniqueName, family);
			if (l != null)
	 ...
	 */
	private DynamicValueProperty<?> getFromFamily(String uniqueName, Class<? extends DynamicValueProperty<?>> type) {
		if (type == null)
	...
	public RemoteInfo asyncLinkProperty(RemoteInfo name, Class<?extends DynamicValueProperty<?>> type, LinkListener<?> l) throws InstantiationException, RemoteException{
		...
		try {
			Class<?extends SimpleProperty<?>> impClass = getPlugInstance().getPropertyImplementationClass(type, name.getRemoteName());
			...
			if (property == null) {
				...
				try {
					property = (DynamicValuePropertyImpl<?>)impClass.getConstructor(String.class, PropertyContext.class).newInstance(name.getRemoteName(), family);
					family.add(property);
					newProp=true;
				} catch (Exception e) {
	 ...
	 */
	private void connect(String uniqueName, Class<?extends SimpleProperty<?>> type, Class<?extends SimpleProperty<?>> implementationType, DynamicValuePropertyImpl<?> property)throws ConnectionException{
		// creates proxy implementation
		PropertyProxy proxy = null;
		DirectoryProxy<?> dir = null;
		try {
			Class<?extends PropertyProxy<?,?>> proxyImplType = getPlugInstance().getPropertyProxyImplementationClass(type, implementationType, uniqueName);

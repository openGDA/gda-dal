 ...
 *
 */
public class MonitorProxyWrapper<T, P extends SimpleProperty<T>> implements ResponseListener<T>, ExpertMonitor, DynamicValueMonitor, Suspendable{
	...
	private DynamicValueAdapter internalListener = new DynamicValueAdapter<T, P>() {
		...
		public void conditionChange(DynamicValueEvent<T, P> event) {
			if (dvl != null)
				...
				DynamicValueListener<T,P>[] dvlArray = (DynamicValueListener<T,P>[])dvls.toArray();
	 ...
	 */
	public MonitorProxyWrapper(P property, DynamicValueListener<T,P> listener){
	 ...
	 */
	public MonitorProxyWrapper(P property, DynamicValueListener<T,P> listener, Map<String, Object> parameters){
		dvl = listener;
	...
	public void responseError(ResponseEvent<T> event){
		...
		// TODO better message
		DynamicValueEvent<T, P> dynamicEvent = new DynamicValueEvent<T,P>(property,property, response.getValue(), lastCondition, response.getTimestamp(), ERROR_MESSAGE, response.getError());
